...
dae::AnalogStickMoveComponent::AnalogStickMoveComponent(GameObject *pOwner, const std::uint32_t gamepadIndex, const float speed, const float deadzoneScale)
    : Component(pOwner), m_gamepadIndex(gamepadIndex), m_speed(speed), m_deadzoneScale(deadzoneScale)
{
}

void dae::AnalogStickMoveComponent::Update()
{
    const auto gamepadState = InputManager::GetInstance().GetGamepadState(m_gamepadIndex);
    if (!gamepadState)
    {
        return;
    }
